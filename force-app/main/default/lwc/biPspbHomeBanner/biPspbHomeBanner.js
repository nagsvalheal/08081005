...
import RETURN_THE_ACCOUNTS from '@salesforce/apex/BI_PSP_CurrentUser.returnTheAccounts';
import GET_ENROLLEE_RECORDS from '@salesforce/apex/BI_PSP_CurrentUser.getEnrolleeRecords';
...
export default class BiPspbHomeBanner extends LightningElement {
	...
	accounts;
	enrolleRec;
	@wire(RETURN_THE_ACCOUNTS)
	wiredReturnAccount({ data }) {
		let globalThis = window;
		try {
			if (data) {
				this.accounts = data;
	...
	@wire(GET_ENROLLEE_RECORDS)
	wiredCEnrolleeRecords({ data }) {
		let globalThis = window;
		try {
			if (data) {
				this.enrolleRec = data;
				this.handleResult(data);
			}
		} catch (err) {
			// Handle error or show an error message
