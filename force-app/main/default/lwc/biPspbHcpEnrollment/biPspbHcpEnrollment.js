    ...
    handlePicklistValues({ data, errors }, targetField) {
        try {
            if (data) {
                ...
                this[targetField] = undefined; // Since it's taken from the schema builder, error message will not be displayed and it will be undefined
...
calculateAge(SELECTED_DATE_OBJ, CURRENT_DATE) {
	...
	validateDOB() {
		if (!this.leadFields.dob) {
			...
			this.dobErrorMessage = false;
			if (this.error === true || this.patientvalidateForm) {
				this.setFieldErrorStyles();
				return false;
			} 
				this.resetFieldStyles();
	...
	hcpIdOnChange(event) {
		...
		this.searchResultTwo = this.picklistOrderedTwo
			.filter(
				(element) => element?.label !== null && element?.label !== undefined
			)
			.filter((picklistOptionTwo) =>
		...
		this.searchResultEmptyTwo = this.searchResultTwo.length === 0 ;

		if (this.searchResultEmptyTwo) {
			this.physicianNameInputDisabled = true;
			this.SearchIdErr();
			this.addNewHcpSectionClass = "addNewHcpSection-disable";
		}
